...
class SolutionAnalyzer {
    ...
    async findProjectFolders(rootPath) {
        const folders = [];
        try {
            ...
            for (const [name, type] of entries) {
                if (type !== vscode.FileType.Directory)
                    continue;
                if (name.startsWith('.') || name === 'node_modules' || name === 'bin' || name === 'obj' || name === 'dist' || name === 'build')
                    continue;
    ...
    async analyzeSingleProject(projectPath, language) {
    ...
    async analyzeFile(file, language, projectName) {
        const elements = [];
        try {
    ...
    extractNamespace(filePath, language) {
        try {
    ...
    extractDependencies(lines, language) {
        const deps = [];
        ...
        switch (language) {
            case 'csharp':
                ...
                if (usingMatch)
                    deps.push(...usingMatch.map(u => u.replace('using ', '')));
                break;
            case 'javascript':
            case 'typescript':
                const importMatch = content.match(/import\s+(?:.*?\s+from\s+)?['"]([^'"]+)['"]/g);
                if (importMatch)
                    deps.push(...importMatch.map(i => i.replace(/import\s+(?:.*?\s+from\s+)?['"]|['"]/g, '')));
                const requireMatch = content.match(/require\(['"]([^'"]+)['"]\)/g);
                if (requireMatch)
                    deps.push(...requireMatch.map(r => r.replace(/require\(['"]|['"]\)/g, '')));
                break;
            case 'python':
                const importRe = content.match(/^(?:from\s+[\w.]+\s+)?import\s+([\w, ]+)/gm);
                if (importRe)
                    deps.push(...importRe.map(i => i.replace(/^(?:from\s+[\w.]+\s+)?import\s+/, '').trim()));
                break;
            case 'java':
                const importJa = content.match(/import\s+([A-Za-z0-9_.]+)/g);
                if (importJa)
                    deps.push(...importJa.map(i => i.replace('import ', '')));
                break;
            case 'go':
                const goImport = content.match(/"[^"]+"/g);
                if (goImport)
                    deps.push(...goImport.map(i => i.replace(/"/g, '')));
                break;
        }
        return [...new Set(deps)];
    }
    ...
    isValidElementName(name) {
        ...
        if (name.match(/^\d/))
            return false;
        if (['ControllerBase', 'Component', 'Module', 'App'].includes(name))
            return true;
        return true;
    }
    categorizeElement(name, type, namespace, language) {
    ...
    groupByNamespace(elements, projectName) {
    ...
    async findDependencies(projectPath, language) {
        const deps = [];
        switch (language) {
            case 'csharp':
                try {
                    const csprojFiles = await vscode.workspace.findFiles(new vscode.RelativePattern(projectPath, '*.csproj'), '', 1);
            ...
            case 'typescript':
                try {
                    const pkgFiles = await vscode.workspace.findFiles(new vscode.RelativePattern(projectPath, 'package.json'), '', 1);
            ...
            case 'python':
                try {
                    const reqFiles = await vscode.workspace.findFiles(new vscode.RelativePattern(projectPath, '{requirements.txt,pyproject.toml,setup.py}'), '', 10);
            ...
            case 'java':
                try {
                    const pomFiles = await vscode.workspace.findFiles(new vscode.RelativePattern(projectPath, '{pom.xml,build.gradle}'), '', 1);
            ...
            case 'go':
                try {
                    const goModFiles = await vscode.workspace.findFiles(new vscode.RelativePattern(projectPath, 'go.mod'), '', 1);
            ...
            case 'rust':
                try {
                    const cargoFiles = await vscode.workspace.findFiles(new vscode.RelativePattern(projectPath, 'Cargo.toml'), '', 1);
